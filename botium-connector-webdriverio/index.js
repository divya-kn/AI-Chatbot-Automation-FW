...
const Capabilities = {
  ...
  WEBDRIVERIO_LOGIN:'WEBDRIVERIO_LOGIN',
  WEBDRIVERIO_CHATBOT_ICON: 'WEBDRIVERIO_CHATBOT_ICON',
  ...
  WEBDRIVERIO_OUTPUT_ELEMENT_TEXT_1: 'WEBDRIVERIO_OUTPUT_ELEMENT_TEXT_1',
  WEBDRIVERIO_OUTPUT_ELEMENT_TEXT_1_NESTED: 'WEBDRIVERIO_OUTPUT_ELEMENT_TEXT_1_NESTED',
  ...
  PASSWORD:'PASSWORD',
  WEBDRIVERIO_INPUT_NAVIGATION_BUTTONS :'WEBDRIVERIO_INPUT_NAVIGATION_BUTTONS',
  WEBDRIVERIO_AUTHENTICATION: 'WEBDRIVERIO_AUTHENTICATION',
  ...
  WEBDRIVERIO_CHROME_ARG:'WEBDRIVERIO_CHROME_ARG'
};
...
const loginDefault = async(container,browser) => {
  browser.pause(4000)
  //const loginSelector = await browser.$(container.caps[Capabilities.WEBDRIVERIO_LOGIN]);
  //const userIdSelector = await browser.$(container.caps[Capabilities.WEBDRIVERIO_USERNAME]);
  //const passwdSelector = await browser.$(container.caps[Capabilities.WEBDRIVERIO_PASSWORD]);
  //const signClickSelector = await browser.$(container.caps[Capabilities.WEBDRIVERIO_AUTHENTICATION]);
  const loginSelector = container.caps[Capabilities.WEBDRIVERIO_LOGIN]
  const userIdSelector = container.caps[Capabilities.WEBDRIVERIO_USERNAME]
  const passwdSelector = container.caps[Capabilities.WEBDRIVERIO_PASSWORD]
  const signClickSelector = container.caps[Capabilities.WEBDRIVERIO_AUTHENTICATION]
  const userName = container.caps[Capabilities.USERNAME]
  const password = container.caps[Capabilities.PASSWORD]
  const loginTimeout = container.caps[Capabilities.WEBDRIVERIO_LOGIN_TIMER] || 10000
  await browser.pause(loginTimeout/5)
  if (loginSelector){
    try{
      const loginClick = await container.findElement(loginSelector)
      await loginClick.waitForClickable({timeout:loginTimeout})
      await loginClick.click()
      /////////////////////////////////
      const userIdInput = await container.findElement(userIdSelector)
      await userIdInput.waitForEnabled({timeout:loginTimeout})
      console.log("here in Userid")
      await userIdInput.setValue(userName)
      //////////////////////////////////
      const passwdInput = await container.findElement(passwdSelector)
      await passwdInput.waitForEnabled({timeout:loginTimeout})
      console.log("here in Password")
      await passwdInput.setValue(password)
      await browser.pause(loginTimeout/5)
      /////////////////////////////////
      const signInClick = await container.findElement(signClickSelector)
      await signInClick.waitForClickable({timeout:loginTimeout})
      console.log("here in signin")
      await signInClick.click()
    }
    catch (err) {
      console.log(`Failed inside login part : ${err}`)}}
  else{
    console.log("failed to load login selector ")
  }
  await browser.pause(loginTimeout)
};
  ...
  try {
    if (container.caps[Capabilities.WEBDRIVERIO_INPUT_NAVIGATION_BUTTONS]) {
      ...
      for (const [i, clickSelector] of clickSelectors.entries()) {
        console.log(`openBotDefault - trying to click on element #${i + 1}: ${clickSelector}`)
        const clickElement = await container.findElement(clickSelector)
        await clickElement.waitForClickable({ timeout: 60000 })
    ...
    console.log(`Input element ${inputElementSelector} is visible`)}
  catch(err){
    const startChatSelector = container.caps[Capabilities.WEBDRIVERIO_CHATBOT_ICON]
    const startChatTimeout = container.caps[Capabilities.WAITFORBOT] || 60000;
    const startChat = await container.findElement(startChatSelector)    
    //await browser.pause(startChatTimeout/10)
    await startChat.waitForClickable({timeout:startChatTimeout})
    try{
      ...
      const inputElementSelector = container.caps[Capabilities.WEBDRIVERIO_INPUT_ELEMENT]
      const inputElementVisibleTimeout = container.caps[Capabilities.WEBDRIVERIO_INPUT_ELEMENT_VISIBLE_TIMEOUT] || 1000
      
      const inputElement = await container.findElement(inputElementSelector)
      await inputElement.waitForDisplayed({ timeout: inputElementVisibleTimeout })
      console.log(`Input element ${inputElementSelector} is visible`)}
    catch(err){
      console.log("err: element not interactable")}
  }
}
  ...
  const inputElementSendButtonSelector = container.caps[Capabilities.WEBDRIVERIO_INPUT_ELEMENT_SENDBUTTON]
    if (msg.buttons && msg.buttons.length > 0) {
      try{
        ...
        //console.log("text",msg.buttons[0].text)
        ...
        console.log(`Waiting for button element to be visible from the received text from bot`)
        ...
        //console.log("text",qrView.button.textlower)
        console.log(`Error message : ${err}`)
        const inputElement = await container.findElement(inputElementSelector)
        await inputElement.waitForEnabled({ timeout: inputElementVisibleTimeout })
        debug(`input element ${inputElementSelector} is visible, simulating input: "${msg.buttons[0].payload}"`)
      if (inputElementSendButtonSelector) {
        debug(`input element ${inputElementSelector} is visible, simulating input: "${msg.buttons[0].payload }" (with Send button ${inputElementSendButtonSelector})`)
        await inputElement.setValue([...msg.buttons[0].payload])
        ...
        console.log(`No buttons to be clicked please check input element button declaration and received message from bot`)
        console.log(`Error message : ${err}`)}
      }
    } else {
      const inputElement = await container.findElement(inputElementSelector)
...
const getBotMessageDefault = async (container, browser, element, html) => {
  ...
  try{
    ...
    console.log(`chat element is been changed ${err}`)
    if (container.caps[Capabilities.WEBDRIVERIO_OUTPUT_ELEMENT_TEXT_1]) {
    if (isNested(Capabilities.WEBDRIVERIO_OUTPUT_ELEMENT_TEXT_1_NESTED, true)) {
      const textElement = await element.$(container.caps[Capabilities.WEBDRIVERIO_OUTPUT_ELEMENT_TEXT_1])
      if (textElement) {
        botMsg.messageText = await textElement.getText()
      }
    } else {
      const textElement = await container.findElement(container.caps[Capabilities.WEBDRIVERIO_OUTPUT_ELEMENT_TEXT_1])
      if (textElement) {
        botMsg.messageText = await textElement.getText()
      }
    }
  ...
  if (isNested(Capabilities.WEBDRIVERIO_OUTPUT_ELEMENT_BUTTONS_NESTED, true)) {
    ...
    try{
      buttonElements = await container.findElements(container.caps[Capabilities.WEBDRIVERIO_OUTPUT_ELEMENT_BUTTONS] || './/button | .//a[@href] | BUTTONS')
    }catch(err){
      console.log(`Buttons eliment not visible ${err}`)}
    }
...
class BotiumConnectorWebdriverIO {
  ...
  async Validate () {
    ...
    if (!this.caps[Capabilities.LOGIN] && !this.caps[Capabilities.WEBDRIVERIO_LOGIN]) throw new Error('WEBDRIVERIO_LOGIN or WEBDRIVERIO_USERNAME or WEBDRIVERIO_PASSWORD capability required')
    ...
    this.login = this._loadFunction(Capabilities.WEBDRIVERIO_OPENBROWSER, loginDefault)
  ...
  async Start () {
    ...
    try {
      ...
      const Args = this.caps[Capabilities.WEBDRIVERIO_CHROME_ARG]
      console.log(Args)
      if (this.caps[Capabilities.WEBDRIVERIO_START_CHROMEDRIVER]) {
      ...
      if (this.caps[Capabilities.LOGIN] == true){
        await this.login(this,this.browser)
        console.log("login turned ON")
      }else{
        console.log("login turned OFF")
      }
      //await retry({times: 3, interval: 200},function(err, result) {
      await this.openBot(this, this.browser) // do something with the result
      //});
  ...
  _loadFunction (capName, defaultFunction) {
    if (!this.caps[capName]) {
      try{
        console.log(`cap name is ${capName}`)
